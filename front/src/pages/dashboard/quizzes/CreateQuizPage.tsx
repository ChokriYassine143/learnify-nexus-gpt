...
const CreateQuizPage: React.FC = () => {
  ...
  useEffect(() => {
    if (quizData.courseId) {
      const selectedCourse = courses.find(c => c.id === quizData.courseId);
      setModules(selectedCourse?.modules || []);
      setQuizData(prev => ({ ...prev, moduleId: "", lessonId: "" }));
      setLessons([]);
    }
  ...
  useEffect(() => {
    if (quizData.moduleId) {
      const selectedModule = modules.find(m => m.id === quizData.moduleId);
  ...
  const handleSaveQuiz = async () => {
    ...
    if (!/^[a-f\d]{24}$/i.test(quizData.courseId)) {
      toast({
        title: "Cours invalide",
        description: "Veuillez sélectionner un cours valide.",
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      const newQuiz: any = {
        ...
        moduleId: quizData.moduleId || undefined,
      ...
      console.log('Quiz payload being sent:', newQuiz);
      ...
      if (error.response) {
        ...
        toast({
          title: 'Backend Error',
          description: error.response.data.message || 'Unknown backend error',
          variant: 'destructive'
        });
      } else {
        toast({
                  ...
                  >
                    ...
                    <SelectContent>
                      {courses.map((course, courseIndex) => (
                        <SelectItem key={course.id || `course-${courseIndex}`} value={course.id}>
                          {course.title}
                        </SelectItem>
                      ))}
                    </SelectContent>
                    ...
                    >
                      ...
                      <SelectContent>
                        {modules.map((module, moduleIndex) => (
                          <SelectItem key={module.id || `module-${moduleIndex}`} value={module.id}>
                            {module.title}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    ...
                    >
                      ...
                      <SelectContent>
                        {lessons.map((lesson, lessonIndex) => (
                          <SelectItem key={lesson.id || `lesson-${lessonIndex}`} value={lesson.id}>
