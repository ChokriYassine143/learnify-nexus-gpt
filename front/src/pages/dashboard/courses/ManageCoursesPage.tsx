...
const ManageCoursesPage: React.FC = () => {
    ...
    const loadCourses = async () => {
      ...
      try {
        ...
        if (isAdmin) {
          ...
          loadedCourses = allCoursesRaw.map(course => ({ ...course, id: course.id || course._id }));
        } else if (isTeacher && user) {
          const allCoursesRaw = await courseService.getAllCourses();
          const allCourses = allCoursesRaw.map(course => ({ ...course, id: course.id || course._id }));
          loadedCourses = allCourses.filter(course =>
            course.instructor === user.id ||
            course.instructor === user._id ||
            (typeof course.instructor === 'object' && (course.instructor.id === user.id || course.instructor._id === user.id || course.instructor._id === user._id))
          );
        } else {
          loadedCourses = [];
        }
  ...
  const basePath = isTeacher ? "/dashboard/teacher/courses" : "/dashboard/courses";
                ...
                <Card key={course.id} className="overflow-hidden hover:shadow-lg transition-all group border shadow-md">
                  ...
                  <CardHeader className="p-4 pb-2 pt-3">
                    ...
                    <CardDescription className="flex items-center gap-1 text-sm">
                      <span>{course.modules.reduce((acc, m) => acc + m.lessons.length, 0)} leçons</span>
                      <span>•</span>
